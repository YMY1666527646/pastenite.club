...
void Utilities::Log2(const char* fmt ...)
{
	SetConsoleColor(FOREGROUND_INTENSE_YELLOW);
	printf("[pastenite.club]");
	SetConsoleColor(FOREGROUND_INTENSITY);
	printf(" welcome back, %s", getenv("USERNAME")); // gets username from C: path, and prints something like [pastenite.club] welcome back, negro
}
